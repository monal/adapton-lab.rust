...
// CSCI 7000 class project - Monal Narasimhamurthy
// Incremental mergesort using list of lists
pub mod mergesort_lol {
  use super::*;
  use std::hash::Hash;
  use std::fmt::Debug;
  use std::rc::Rc;
    ...
    (a:(), inp:List<X>) -> List<List<X>>
  {
    match inp {
      ...
      // Given an element x from the input list, the corresponding singleton list element should be
      // List::Cons(x, Box::new(List::Name(nm1, Box::Art(cell(nm2, List::Nil)))))
      // In order to generate names nm1, nm2 we look ahead in the input list and fork names
      // from the next List::Name cell.
      // The singleton list needs to have names because when two singleton lists are merged,
      // the resulting list needs to have names for efficient memoization.
      List::Cons(x, xs) => {
        match *xs.clone() {
    	  List::Name(nm, ys) => {
    ...
    (a:(), lst_a: List<X>, lst_b: List<X>) -> List<X>
  {
    match (lst_a.clone(), lst_b.clone()) {
      ...
      // While iterating over the input lists, if we observe a List::Cons cell, then we make sure that we 
      // read the corresponding List::Name and List::Art cells as well, so that the order of the names and references 
    ...
    (a:(), inp: List<List<X>>) -> List<List<X>>
  {
    match inp.clone() {
      ...
      // If a List::Cons cell is encountered, then the next List::Cons cell is identified 
      // and the two sublists are merged.
      List::Cons(x, xs) => {
        match *xs.clone() {
  	  ...
  	  List::Name(n1, zs) => {
  	    ...
  	    match force(&x_cell){
		    ...
		    let rest = list_of_list_merge((), force(&yCell));
  	  	    let merged_lists = list_merge((), x, y);
		    let (nm1, nm2) = name_fork(name_pair(n1,n2));
  ...
  //Merge list of lists recursively:
  ...
  { 
    ...
    let res = mergesort_list_of_lists_rec((), singletons);
  ...
  pub struct RunMergesort { } 
    impl Compute<List<usize>, List<usize>> for RunMergesort {
    fn compute(inp:List<usize>) -> List<usize>{ mergesort_list_of_lists((), inp) }
  }
  ...
  impl<S> Generate<List<usize>> for UniformPrepend<List<usize>,S> {
    fn generate<R:Rng>(rng:&mut R, params:&GenerateParams) -> List<usize> {
      ...
      for i in 0..params.size {
        if i % params.gauge == 0 {
          l = List::Art(cell(name_of_usize(i), l));
          l = List::Name(name_of_usize(i), Box::new(l));
        } else { } ;
        let elm : usize = rng.gen() ;
        let elm = elm % ( params.size * 100 ) ;
        l = List::Cons(elm,  Box::new(l));
        if i % params.gauge == 0 {
...
pub mod hammer_s17_hw1 {
    ...
    where F:Fn(X) -> bool
  {
    match inp {
      ...
      List::Cons(x, xs) => {
        ...
        match f(x.clone()) {
          true => List::Cons(x, Box::new(rest)),
          false => rest
	}
      },
      List::Name(nm, xs) => {
        memo!(nm.clone() =>> list_filter_name =>> <X,F>, 
          nm:nm, xs:xs 
          ;; 
          f:f)   
      },
      List::Art(xs) => list_filter(force(&xs), f)
    }
  }

  pub fn list_filter_name<X:Eq+Clone+Hash+Debug+'static, F:'static>
    (nm:Name, xs: Box<List<X>>, f:Rc<F>) -> List<X> 
    where F:Fn(X) -> bool
   {    
      let (nm1, nm2) = name_fork(nm);
    ...
    where F:Fn(X) -> bool
  {
    match inp {
      List::Nil => (List::Nil, List::Nil),
      List::Cons(x, xs) => {
        let (rest_left, rest_right) = list_split(*xs, f.clone());
        match f(x.clone()) {
          true => (List::Cons(x, Box::new(rest_left)), rest_right),
          false => (rest_left, List::Cons(x, Box::new(rest_right)))
	}
      },
      List::Name(nm,  xs) => {
        memo!(nm.clone() =>> list_split_name =>> <X,F>,
          nm:nm, xs:xs
          ;;
          f:f)
      },
      List::Art(xs) => list_split(force(&xs), f)
    }
  }

  pub fn list_split_name<X:Eq+Clone+Hash+Debug+'static, F:'static>
    (nm: Name, xs: Box<List<X>>, f:Rc<F>) -> (List<X>, List<X>)
    where F:Fn(X) -> bool
   {
      let (nm1, nm2) = name_fork(nm);
      let (rest_left, rest_right) = list_split(*xs, f);
      let (nm_left, nm_right) = name_fork(nm2.clone());
      (List::Name(nm1.clone(), Box::new(List::Art(cell(nm2.clone(), rest_left)))), List::Name(nm1, Box::new(List::Art(cell(nm2, rest_right)))))
   }
    ...
    (inp:List<X>, acc:List<X>) -> List<X>
  {
    match inp {
      List::Nil => acc,
      List::Cons(x, xs) => {
	list_reverse_rec(*xs, List::Cons(x, Box::new(acc)))
      },
      List::Name(nm, xs) => {
        memo!(nm.clone() =>> list_reverse_name :: <X>,
          nm:nm, xs:xs, acc:acc)
      },
      List::Art(xs) => list_reverse_rec(force(&xs), acc)
    }
  }

  pub fn list_reverse_name<X:Eq+Clone+Hash+Debug+'static>
    (nm:Name, xs: Box<List<X>>, acc:List<X>) -> List<X>
  {
    let (nm1, nm2) = name_fork(nm);
    let rest = list_reverse_rec(*xs, acc);
    List::Name(nm1, Box::new(List::Art(cell(nm2, rest))))
  }
    ...
    (inp: List<List<X>>) -> List<X>
  {
    match inp {
      List::Nil => List::Nil,
      List::Cons(x, xs) => {
	let rest = list_join(*xs);
        list_join_rec(x, rest)
      },
      List::Name(nm, xs) => {
        memo!(nm.clone() =>> list_join_name :: <X>,
          nm:nm, xs:xs)
      },
      List::Art(xs) => list_join(force(&xs))
    }
  }

  pub fn list_join_rec<X:Eq+Clone+Hash+Debug+'static>
    (inp_list1: List<X>, inp_list2: List<X>) -> List<X>
  {
    match inp_list1 {
      List::Nil => inp_list2, 
      List::Cons(x, xs) => List::Cons(x, Box::new(list_join_rec(*xs, inp_list2))),
      List::Name(nm, xs) => List::Name(nm, Box::new(list_join_rec(*xs, inp_list2))),
      List::Art(xs) => list_join_rec(force(&xs), inp_list2)
    }
  }

  pub fn list_join_name<X:Eq+Clone+Hash+Debug+'static>
    (nm:Name, xs: Box<List<List<X>>>) -> List<X>
  {
    let (nm1, nm2) = name_fork(nm);
    let rest = list_join(*xs);
    List::Name(nm1, Box::new(List::Art(cell(nm2, rest))))
    ...
    (inp: List<X>) -> List<List<X>>
  {
	...
	List::Cons(List::Cons(x, Box::new(List::Nil)), Box::new(rest))
      ...
      List::Name(nm, xs) => { 
        memo!(nm.clone() =>> list_singletons_name :: <X>,
          nm:nm, xs:xs)
      },
      List::Art(xs) => list_singletons(force(&xs))
    }
 ...
 pub fn list_singletons_name<X:Eq+Clone+Hash+Debug+'static>
    (nm:Name, xs: Box<List<X>>) -> List<List<X>>
  {
    let (nm1, nm2) = name_fork(nm);
    let rest = list_singletons(*xs);
    List::Name(nm1, Box::new(List::Art(cell(nm2, rest))))
...
pub fn all_labs() -> Vec<Box<Lab>> {
  return vec![
    ...
    labdef!(name_of_str("project-mergesort-lol"),
            Some(String::from("")),
            mergesort_lol::List<usize>, usize,
            mergesort_lol::List<usize>,
            mergesort_lol::UniformPrepend<_,_>,
            mergesort_lol::RunMergesort)
      ,
